...
TA2CylMwpc::TA2CylMwpc( const char* name, TA2System* apparatus ) : TA2WireChamber(name, apparatus)
{
  ...
  //Track Hits 		Chris Mullen 8/4/16
...
//Chris Mullen 08/04/16 Adding lines called hitpositionwc1 or the like to extract the data for recoil Polarimeter v2. Need to understand what TooManyTracks,GetType and GetPosition do. Need to understand the behaviour of r[0], r[1] in the loops. I think the loop accepts the first hit in WC1 and puts it into r[0] then the second loop puts the first hit in WC2 into r[1] then builds tracks and saves them from here on. Then the second loop goes through all the hits in the second wire chamber, WC2, and builds tracks etc for all these with the first r[0]. Then r[0] is changed to the second and all the tracks are built for all the r[1]'s and then the third r[0] and so on ... where in that last sentence I have just called r[0] and r[1] the hits, see prev sentence for proper formation. Need to look at syntax of fInters[j][k]
 // fInters[iCh][iInter] is the correct syntax so it is chamber number(wire chamber no.) and intersection number so hit number.
  ...
  //Chris Mullen 8/4/16 editing to add hit positions described in comment above. Will make one grouping(array,list something like that) of all hits in chamber one and one array of all hits in chamber 2 for this event( be careful need to make sure there is a more global loop going round all events, my array or storage method will need to be defined globallly and appended to then saved to a root tree, likely need to be a branch with leaves for each component x,y,z). Make sure no cut offs applied before this point in terms of angles allowed.

/*
Int_t j[2];
const TVector3 *HitPos[2];


for (j[0]=0; j[0]<fNinters[0]; ++j[0])
  {
    if (TooManyTracks()) break;
    HitPos[0] = fInters[0][j[0]].GetPosition();

    //Append here	Next steps, look at what get type, intersectionkEWI does to see if can discard still not sure what it does but it checks if fInters is a true interaction so may be safe to get rid of as we want all interactions. However, I will leave it in just in case it has a side effect for the minute. Next step is to track a variable eg fNtracksTrueBest through to the tree and copy most of the process for it. Need to search for this in .h file too. This is the important part where the types are defined. Remember to define types in both MakeTracks and the .h file for each scope you want. Also watch out for reuse of the variables such as r here that may not be deleted from prev loops so may gie us problems.
}
for (j[1]=0; j[1]<fNinters[1]; ++j[1])
    {
      if (TooManyTracks()) break;
      HitPos[1] = fInters[1][j[1]].GetPosition();
      //Append here
